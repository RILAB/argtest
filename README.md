...
3. **Find aberrant mutational load.** All samples should have similar derived-mutation loads after accounting for local mutation-rate variation. [scripts/mutload_masks.py](scripts/mutload_masks.py) simulates mutations once on the input ARG to get a per-window, per-individual simulation-based expected load, then flags any (window, individual) pair whose observed load deviates from expectation by more than `mutload_cutoff`, writing one BED row per outlier window with the affected individuals in column 4. The test is per individual: a diploid/polyploid individual's haploid copies are pooled before testing, so for haploid data it is effectively per sample. [scripts/mutload_summary.py](scripts/mutload_summary.py) gives an HTML diagnostic of residual load after pruning, flagging candidates for whole-sample removal.
4. **Remove affected regions.** Combine the step 1–3 BED masks per chromosome and apply them to each tree sequence with [scripts/trim_regions_single.py](scripts/trim_regions_single.py). This is the pipeline's coordinate-preserving regional filter: removed spans become empty intervals rather than shifting later coordinates. The resolved mutation-rate map is embedded in each filtered tree sequence's metadata.
...
6. **Validate** (optional). [scripts/validation_plots_from_ts.py](scripts/validation_plots_from_ts.py) produces compact QC plots (mutational load, diversity, Tajima's D, folded/unfolded SFS) for the cleaned ARG; compare against the same plots on the originals. Alongside each set of plots it writes the numbers behind them as three tab-separated tables — `windows.tsv` (per-window), `samples.tsv` (per-sample) and `sfs.tsv` (per-frequency-bin) — so any panel can be re-checked or re-plotted without reloading the ARGs.
6b. **Pool across chromosomes** (optional). [scripts/genomewide_expected_vs_observed.py](scripts/genomewide_expected_vs_observed.py) reads every chromosome's `windows.tsv` and plots expected versus observed π and Tajima's D for all windows at once, colouring each point by the recombination rate of that window taken from the HapMap map. The per-chromosome scatters can only show whether one chromosome is off; pooling with a recombination-rate colour axis shows whether the deviation from the 1:1 line *tracks* recombination rate, which is the signature of ARG inference degrading in low-recombination regions. Requires `validation_sim_branch` — without a simulated expectation there is nothing to plot against.
...
8. **Summarise** (optional). [scripts/pipeline_summary.py](scripts/pipeline_summary.py) collects genome-retention statistics across all steps (mean ± SD across replicates), per-individual outlier counts, the step-6b genome-wide expected-vs-observed panels, and the per-chromosome validation plots into one self-contained HTML file. Final retained Mb are measured directly from the filtered ARGs as positive-rate accessible sequence with a non-empty local genealogy, so they exclude both regions masked in the input ARG and regions removed by this pipeline. Retention percentages use the full FASTA-index reference length as the denominator.
...
- [`genomewide_expected_vs_observed.py`](scripts/genomewide_expected_vs_observed.py) — pool every chromosome's `windows.tsv` into one expected-vs-observed scatter per statistic (π and Tajima's D), coloured by each window's length-weighted mean recombination rate. Windows the map does not cover are drawn grey rather than as zero-rate; when the positive rates span more than a decade the colour axis is log-scaled and zero-rate windows are clipped to the smallest positive rate (stated on the colourbar).
