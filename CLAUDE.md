...
Snakemake pipeline for analyzing tree sequences. **The input dataset is configurable — it is not always amaranth.** The dataset is selected by `root_dir` (and `base_name`) in whichever config you pass with `--configfile`; `config/snakemake.yaml` is the documented template. Steps:
...
`merge_replicates` (`merge_treefiles_by_replicate.py`) loads ALL chromosomes of a replicate into memory at once and `.concatenate()`s them, so its peak RAM is far higher than any other step (steps 1-5 each hold a single tree sequence). On large real datasets, whole-genome tables can be much smaller than the merge transient peak; budget generously, often ~96-128 GB per merge or more for larger genomes / higher sample counts.
...
`measure_merge_mem.py` (repo root) measures the actual merge peak RSS for one replicate without writing anything to disk: `python measure_merge_mem.py [REP] [GLOB]` (GLOB has a single `{rep}` field). Use it to size `mem_mb` for a new dataset before a full run.
